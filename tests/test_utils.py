...
class TestConfidenceSequence:
    ...
    def test_cs_radius(self):
        assert np.isclose(
            utils.cs_radius(var=self.var1, t=self.t, t_star=self.t, alpha=.05),
            self.truth,
            rtol=1e-15
        )
    ...
    def test_cs_comparison(self):
        assert np.isclose(
            utils.cs_radius(var=self.var1, t=self.t, t_star=self.t, alpha=.05),
            utils.cs_radius(var=self.var2, t=self.t, t_star=self.t, alpha=.05),
            rtol=1e-15
        )
...
class TestITE:
    ...
    def test_ite_control(self):
        assert utils.ite(2., 0, 0.25) == -8

    def test_ite_treatment(self):
        assert utils.ite(2., 1, 0.5) == 4
...
def test_var():
    assert np.isclose(
        utils.var(outcome=0.75, propensity=0.5),
        2.25,
        rtol=1e-15
    )
